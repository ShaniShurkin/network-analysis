...
from entity_services.technician import BaseTechnician, get_technician_by_condition, \
    get_all_technicians, delete_technician_by_condition
...
@router.post("/create-client/{{name}}")
@handle_app_exceptions
def create_client(request: Request, name: str):
...
@handle_app_exceptions
def add_client_to_current_technician(request: Request, id: int):
    client_id = add_client_to_technician(client_id=id,
